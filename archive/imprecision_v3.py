...
from utils import apply_app_styling, grubbs_test, check_westgard_rules
...
# Sidebar for Outliers Identification and Exclusion
st.sidebar.markdown(
    "<h3>❌ Outliers </h3>",
    unsafe_allow_html=True
)
grubbs_outliers = {
    'grubbs': st.sidebar.checkbox("Identify outliers using Grubbs' test", value=False)
}

if grubbs_outliers['grubbs']:
    exclude_outliers = {
...
def precision_studies(df, selected_analyte, rules_enabled, grubbs_outliers):
    ...
    for (material, analyzer), group in inter_batch_groups:
        ...
        if grubbs_outliers['grubbs']:
            for idx in outlier_indices:
                # Assuming 'group' contains the full data for the current material and analyzer
            ...
            if outlier_details:
                # Combine outlier details into a single string
                outlier_summary = " | ".join(outlier_details)
                st.success(f"Grubb's Test applied. {len(outlier_indices)} outlier(s) identified: {outlier_summary}")
            else:
                st.success(f"Grubb's Test applied to {material} for {analyzer}. No outliers identified.")
