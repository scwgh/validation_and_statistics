...
st.title("♾️ Limit of Detection (LOD) and Limit of Blank (LOB)")
...
if df is not None:
    ...
    for analyte in analyte_columns:
        ...
        lod = round(lob + 1.645 * sd, 5)
        lob = round(mean + 1.645 * sd, 5)
    ...
    st.subheader("♾️ LOB and LOD Results")
    st.dataframe(lod_df)

    # Download button
    lod_csv = lod_df.to_csv(index=False).encode('utf-8')
    st.download_button("⬇️ Download LOB/LOD Results CSV", lod_csv, "lod_results.csv", "text/csv")
